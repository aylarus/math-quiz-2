...
class Quiz:
    def __init__(
        self, parent
    ):  #constructor, The __init__() function is called automatically every time the class is being used to create a new object.

        self.bg_images = Image.open(
            "math2.jpg")  #need to use image if need to resize
        self.bg_image = self.bg_images.resize((450, 350), Image.ANTIALIAS)
        self.bg_image = ImageTk.PhotoImage(self.bg_image)
        #self.bg_image = PhotoImage(file="mpic.jpg")

        #frame set up
        self.quiz_frame = Frame(parent, bg="white", padx=2, pady=2)
        ...
        self.heading_label = Label(self.quiz_frame,
                                   text="Welcome to the Maths Quiz Program ",
                                   font=("Comic Sans MS", "11"),
                                   bg="lightcyan")
        self.heading_label.grid(row=1, padx=20)
...
if __name__ == "__main__":
    ...
    quiz_instance = Quiz(
        root)  #instantiation, making an instance of the class Quiz
